...
namespace Test20311M
{
    ...
    public interface IWriteDisplay
    {
        ...
        void DataWritten(byte[] data, int count, uint index, bool overlap = true);
    }
    ...
    public class PortModel
    {
        ...
        /// Порты
        /// </summary>
        private GeneralPort port1, port2;
        ...
        public int BaudRate
        {
            ...
            set
            {
                ...
                if(IsOpened)
                {
                    ...
                    if (param.twoPorts) { port2.BaudRate = param.baudRate; }
                }
            }
        }
        ...
        public PortModel(IPortDisplay display, Action closePortsAction, Action<string> errorHandler)
        {
            ...
            IsOpened = false;
            IsClosing = false;
            port1 = new GeneralPort();
            port2 = new GeneralPort();
        ...
        /// <param name="readingDelay">Время ожидания конца пакета</param>
        /// <param name="overlap">Признак "перекрываемого" отображения данных</param>
        /// <returns></returns>
        public int WriteAndRead(byte[] writeData, int delay, IReadDisplay readDisplay, int readingDelay = 0, bool overlap = true)
        {
            if (!IsOpened) throw new Exception("Не открыт порт для передачи данных");
            ...
            receiver.Suspend();
            transmitter.Write(port1, writeData, overlap);
            Thread.Sleep(delay);
            receiver.Read(readDisplay, readingDelay);
            receiver.Resume();
            return 0;
        }
        ...
        /// <summary>
        /// Удалить всех зарегистрированных наблюдателей за состоянием портов
        /// </summary>
